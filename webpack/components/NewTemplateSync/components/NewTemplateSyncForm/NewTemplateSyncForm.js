...
const submit = syncType => (formValues, dispatch, props) => {
  const { submitForm, importUrl, exportUrl, history, currentFields, currentLocation, currentOrganization } = props;
  ...
  return submitForm({
    url,
    values: addOrgParams(addLocParams(postValues, currentLocation), currentOrganization),
